...
import "./ExpenseFromData.css";
import { StoreContext } from "../../context/StoreContext";
import Loader from "../../components/Loader/Loader";
const ExpenseFromData = () => {
 ...
 const { setLoading, loading } = useContext(StoreContext);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch(`${url}/api/car/expense-by-date?start=${startDate}&end=${endDate}&state=${state}`);
    ...
    useEffect(() => {
        const total = expense.reduce((sum, expense) => sum + (expense.price || 0), 0);
        setAllPrice(total);
    }, [expense]);
    ...
    return (
        <div className='list add flex-col'>
            ...
            <div ref={printRef} className="print-area">
                <div>
                    <h2 className="text-center mb-2">مشتريات أخرى</h2>
                    <div className="list-table">
                        ...
                        {expense.length === 0 && <p className="text-center text-gray-500">لا توجد نتائج لعرضها</p>}

                        {loading ? <Loader /> : (
                        expense.map((expense, index) => (
                            <div key={expense._id || index} className="list-table-formats">
                                <p>{index + 1}</p>
                                <p className='linkToOneCar'>{ expense.name}
                                </p>
                                <p>{expense.price}</p>
                                <p>{expense.description}</p>
                                <p>{expense.createdAtFormatted}</p>
                            </div>
                        )))}
...
export default ExpenseFromData;
